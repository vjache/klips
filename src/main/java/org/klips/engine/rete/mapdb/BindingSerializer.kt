...
class BindingSerializer(val refs: Collection<Facet.FacetRef<*>>,
                        val tFactory: TupleFactory?) : GroupSerializerObjectArray<Binding>() {
    constructor(refs: Collection<Facet.FacetRef<*>>)
    : this(refs, null)

    val basic = object : ValueSerializer(){
        override fun deserializeEx(typeCodeRaw: Int, input: DataInput2, available: Int): Any {
            val typeCode = typeCodeRaw - 64
            val size = input.readShort().toInt()
            val args = Array(size){ i ->
                deserialize(input, available)
            }
        ...
        override fun serializeEx(output: DataOutput2, tuple: Any) {
    ...
    override fun deserialize(input: DataInput2, available: Int): Binding {
        val m = HashMap<Facet<*>, Facet<*>>(8)
        refs.forEach { m[it] = (basic.deserialize(input,available) as Comparable<Any>).facet }
        return SimpleBinding(m)
    }

    override fun serialize(output: DataOutput2, p1: Binding) {
        refs.forEach {
